const contact = {
    ...
    hide: function(elm){
        ...
        setTimeout(()=>{elm.classList.remove("--hidden");}, 600);
    },
    next_query: function(){
        ...
        setTimeout(()=>{this.show(this.query[(this.index-1)])}, 900);
        this.index++;
    ...
    open: function(){
        ...
        setTimeout(()=>{this.show(this.query[(this.index-1)])}, 1200);
    },
    close: function(){
        setTimeout(()=>{document.querySelector('#js-contact').classList.remove("--visible");}, 600);
        setTimeout(()=>{document.querySelector('#js-overlay').classList.remove("--visible");}, 1200);
        this.hide(this.query[this.index-1]); 
    }
}
