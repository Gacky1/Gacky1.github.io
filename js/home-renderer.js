...
class home_js extends Highway.Renderer {
    ...
    onEnter(){
        ...
        window.onload = ()=>{
            ...
            let home_title_anim = gsap.timeline({delay: .6, defaults:{ease: smooth}});

            home_title_anim.from(".js-hero-title .line-child", {y: "40%", rotateX: "-90deg", stagger: .2, duration: 1.9}, "+=1")
            .from(".c-visual-line", {scaleX: 0, duration: .5}, "-=1.2")
            .fromTo("#v_path", {drawSVG: 0}, {drawSVG: "100%", duration: .5, ease: "power1.out"}, "-=.8")
            .fromTo("#i_path", {drawSVG: 0}, {drawSVG: "100%", duration: .6, ease: "power1.out"}, "-=.7")
            .fromTo("#s_path", {drawSVG: 0}, {drawSVG: "100%", duration: .8, ease: "power1.out"}, "-=.6")
            .fromTo("#u_path", {drawSVG: 0}, {drawSVG: "100%", duration: .9, ease: "power1.out"}, "-=.5")
            .fromTo("#a_path", {drawSVG: 0}, {drawSVG: "100%", duration: 1, ease: "power1.out"}, "-=.4")
            .fromTo("#l_path", {drawSVG: 0}, {drawSVG: "100%", duration: 1.2, ease: smooth}, "-=.3")
            .fromTo("#dot_path", {drawSVG: 0}, {drawSVG: "100%", duration: .3, ease: "power1.out"})
            .from(".c-title .o-title-small .line-child", {y: "100%", duration: .8, stagger: .14}, "-=2.8");
       ...
       // window.addEventListener("resize", resize_card);
        ...
        if(window.innerWidth < 1000){
            let card = document.querySelectorAll(".js-cards > div");
            gsap.from(card[0], {scrollTrigger:{trigger: card[0], scroller: "#js-scroll", end: "bottom 0", ease:  "Power0.easeNone", scrub: true}, y: "-35%"})
            gsap.from(card[1], {scrollTrigger:{trigger: card[1], scroller: "#js-scroll", end: "bottom 0", ease:  "Power0.easeNone",  scrub: true}, y: "-22%"})
            gsap.from(card[2], {scrollTrigger:{trigger: card[2], scroller: "#js-scroll", end: "bottom 0", ease:  "Power0.easeNone",  scrub: true}, y: "-10%"})
        }
...
//         }
        ...
        document.querySelector("#js-work").addEventListener("mousemove", (e)=>{
            gsap.to(cursor, {x: e.clientX-18, y: e.clientY-10, duration: .5})
        })
        ...
        function contact_open(){
            ...
            if(contact.first==true){
                setTimeout(()=>{contact.next_query();}, 1000);
                setTimeout(()=>{
                    contact.next_query();
                }, 4500);
                contact.first=false;
        ...
        let cate = document.querySelector(".c-cate"), num,
        ...
        document.querySelector("#contact_btn").addEventListener("click", ()=>{
            ...
            // console.log(num);
            cate_img.src = new URL('../assets/cate1.png', import.meta.url);
            if(num == 4){
                cate.style = "display: block";
                cate_btn[0].addEventListener("click", ()=>{
                    document.querySelector(".c-cate > div").style.display = "none";
                    cate_img.src = new URL('../assets/cate2.png', import.meta.url);
                    setTimeout(()=>{
                ...
                cate_btn[1].addEventListener("click", ()=>{
                    document.querySelector(".c-cate > div").style.display = "none";
                    cate_img.src = new URL('../assets/cate3.png', import.meta.url);
                    setTimeout(()=>{
                       cate.style = "display: none";
                       contact_open();   
                    }, 3000);
